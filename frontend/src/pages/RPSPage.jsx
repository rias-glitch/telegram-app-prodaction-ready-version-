...
export function RPSPage({ user, setUser, refreshBalance }) {
  ...
  const handleGameResult = async (newGems) => {
    if (setUser && newGems !== undefined) {
      setUser(prev => ({ ...prev, gems: newGems }))
    }
    // Обновляем баланс с сервера после игры
    if (refreshBalance) {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <span>✊</span> Камень Ножницы Бумага
          </h1>
          <p className="text-white/60 text-sm">Победи соперника и выиграй x2</p>
        </div>
      </div>
